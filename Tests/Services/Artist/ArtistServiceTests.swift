...
class ArtistServiceTests: XCTestCase {
  ...
  func testSavingArtists() {
    ...
    waitUntil { done in
      artistService.saveArtists(artists).then { _ -> Void in
  ...
  func testGettingArtistsNeedingTagsUpdate() {
    ...
    waitUntil { done in
      self.realmService.save(artists1 + artists2).then { _ -> Void in
  ...
  func testUpdatingArtistWithTags() {
    ...
    waitUntil { done in
      artistService.updateArtist(artist, with: tags).then { _ -> Void in
  ...
  func testCalculatingTopTagsForArtist() {
    ...
    waitUntil { done in
      artistService.calculateTopTags(for: artist, using: calculator).then { _ -> Void in
        let expectedArtist = self.realmService.object(ofType: Artist.self, forPrimaryKey: artist.name)
  ...
  func testGettingSimilarArtistsWithFailure() {
    ...
    let artist = ModelFactory.generateArtist()
    waitUntil { done in
      artistService.getSimilarArtists(for: artist).then { _ in
        fail()
      }.catch { _ in
        done()
      }
